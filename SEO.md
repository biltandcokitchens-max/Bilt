...
### ✅ Central Queensland location pages — **DONE** (24 Aug 2026)

Six pages, generated by `_build_locations.py`. Hub-and-spoke, so the towns reinforce each other instead of competing for the same terms.

| URL | Angle that makes it non-duplicate | Words |
|---|---|---|
| `/kitchens/central-queensland/` | Hub. Why regional QLD pays twice for a kitchen — once for cabinetry, once for distance | 548 |
| `/kitchens/rockhampton/` | High-set Queenslander housing stock; under-house conversions needing a second kitchen | 654 |
| `/kitchens/gladstone/` | Industrial workforce → investment/rental refits; durability as the spec, not a nicety | 642 |
| `/kitchens/yeppoon/` | Salt air attacks hardware first; split holiday-let vs downsizer market | 647 |
| `/kitchens/emerald/` | 270 km inland — distance is the real cost; 90s/00s stock now due | 611 |
| `/kitchens/biloela/` | Small town, short supplier list; waiting is the biggest cost | 524 |

**The doorway-page test.** Near-identical location pages with the town swapped get demoted. Measured pairwise 5-gram overlap across all six: **worst pair 14.2%**, most pairs 8–12%. Every page has its own angle, prose and four town-specific FAQs.

**Per page:** unique title (58–75 chars) and description (121–153 chars), canonical, OG + Twitter, breadcrumb, and JSON-LD carrying `WebPage` + `BreadcrumbList` + `Service` (with `areaServed`) + `FAQPage`. All validated as parsing.

**Still no `LocalBusiness`, still no invented NAP.** Delivery cost and lead time per town are genuinely unknown, so each page carries a visible `[TBC]` block instead of a fabricated figure. Same for the phone and ABN in every footer.

### ✅ Supporting work — **DONE** (24 Aug 2026)

| # | Change | File |
|---|---|---|
| 16 | **`sitemap.xml` generated** — 8 URLs, built from the page set so it cannot go stale by hand | `sitemap.xml` |
| 17 | **`css/pages.css`** — content-page stylesheet reusing the landing page's exact tokens and type ramp. These pages are plain static HTML, *not* the `<x-dc>` runtime: a search-landing page should paint on the first byte rather than after a runtime boots | `css/pages.css` |
| 18 | **Three legal pages built** — `/legal/privacy/`, `/legal/terms/`, `/legal/warranty/`. The footer links I created in change #12 were pointing at 404s across seven pages; a 404 is worse than the `href="#"` it replaced | `_build_legal.py` |
| 19 | Legal pages set `noindex, follow` and excluded from the sitemap — they are drafts, and an unfinished legal page must never be what a search result lands on | `_build_legal.py` |

**Privacy is substantially real** — most of it is a factual description of what this codebase does (measurement layer inert, bcrypt hashes, JWT in the browser, planner designs never leaving the client, no ad pixels). **Terms and warranty are deliberately not drafted** — consumer-law sensitive, per handoff §6.

**Verified across all 12 routes:** every page 200, **0 broken links out of 24 URLs checked**, legal pages confirmed `noindex` and absent from the sitemap. Rockhampton at 375×812: 65 elements, **0 contrast failures**, no tap target under 44 px, no overflow.
...
| Priority | Task | Notes |
|---|---|---|
| ~~P1~~ | ~~Generate `sitemap.xml`~~ | ✅ Done — 8 URLs, generated from the page set. |
...
| **P1** | **Finish `/legal/privacy/`** | ✅ Page built, `noindex`. Two `[TBC]` blocks left (data residency/retention; privacy contact + postal address). Must be finished and indexable *before* the measurement ID goes in. |
